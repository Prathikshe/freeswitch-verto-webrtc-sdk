/**
 * ============================================================
 *  verto-sdk.js  — Complete Agent Phone SDK
 *  Bundles: FSRTC + JsonRpcClient + $.verto (from verto-min.js)
 *  + Direct Login (NO provisioning API) + CallStateChange helpers
 *
 *  HOW TO USE IN ANY UI:
 *  ─────────────────────
 *  1. Include jQuery 2.x / 3.x  (CDN or local)
 *  2. Include this file:  <script src="verto-sdk.js"></script>
 *  3. Define a <audio id="webcam" autoplay></audio> element
 *  4. Call  VertoPhone.configure(config)
 *  5. Call  VertoPhone.login(params, onStatusList)
 *
 *  CONFIG (Optional)
 *  ─────────────────
 *  {
 *    socketUrl : "wss://host:8082" or "ws://host:8081",  // Verto WS URL
 *    tag       : "webcam",           // <audio> element id
 *    ringFile  : "sounds/bell_ring2.wav",
 *    useStun   : true,
 *    videoParams: { minWidth:"1280", minHeight:"720", minFrameRate:30 },
 *    audioParams: { googAutoGainControl:false, googNoiseSuppression:false,
 *                   googHighpassFilter:false }
 *  }
 *
 *  LOGIN PARAMS (Replaces provisioning API)
 *  ────────────────────────────────────────
 *  {
 *    agentId     : "1001",                  // REQUIRED
 *    domain      : "sip.myserver.com",      // REQUIRED
 *    password    : "secret123",             // REQUIRED
 *
 *    displayName : "Agent 1001",            // OPTIONAL
 *    socketUrl   : "wss://host:8082",       // OPTIONAL (overrides config)
 *
 *    statusList  : [                        // OPTIONAL agent states (if not provided, defaults to login/logout only)
 *      { status_text:"login",          status_number:"*81{agentId}{password}" },
 *      { status_text:"Break",          status_number:"*82" },
 *      { status_text:"Ready for call", status_number:"*83" }, 
 *      { status_text:"Logout",         status_number:"*84" }
 *    ]
 *  }
 *
 *  EVENT HANDLERS (via internal callbacks)
 *  ───────────────────────────────────────
 *  onStatusList(statuses)
 *      Called after login if statusList is provided
 *
 *  Internal Events (handled inside SDK):
 *    onWSLogin        → connection success/failure
 *    onWSClose        → socket disconnected
 *    onDialogState    → call state changes
 *
 *  PUBLIC METHODS (VertoPhone.*)
 *  ─────────────────────────────
 *  VertoPhone.configure(config)
 *      Set global config like socketUrl
 *
 *  VertoPhone.login(params, onStatusList)
 *      Main entry point (NO API call)
 *      Builds SIP credentials and connects
 *
 *  VertoPhone.connect(config)
 *      Low-level: connect with full config object directly
 *
 *  VertoPhone.call(number)          Outbound call
 *  VertoPhone.hangup()              Hang up active call
 *  VertoPhone.logout()              Logout + disconnect
 *  VertoPhone.getStates()           → { names, codes, logoutCode }
 *
 *  (If extended:)
 *  VertoPhone.answer()              Answer incoming call
 *  VertoPhone.reject()              Reject incoming call
 *  VertoPhone.hold()                Toggle hold
 *  VertoPhone.transfer(number)      Blind transfer
 *  VertoPhone.dtmf(digit)           Send DTMF
 *  VertoPhone.changeState(code)     Change agent state
 *
 * ============================================================
 */
...
})(jQuery, document);/*! jQuery JSON plugin 2.4.0 | code.google.com/p/jquery-json */
...
return'"'+str+'"';};}(jQuery));
// ─── 2. verto-min.js (FSRTC + JsonRpcClient + $.verto) ─────────────────────
(function($) {
    function findLine(sdpLines, prefix, substr) {
        console.log("LOG verto min js findLine")
        return findLineInRange(sdpLines, 0, -1, prefix, substr);
    }

    function findLineInRange(sdpLines, startLine, endLine, prefix, substr) {
        console.log("LOG verto min js findLineInRange")
    ...
    function getCodecPayloadType(sdpLine) {
        console.log("LOG verto min js getCodecPayloadType")
    ...
    function setDefaultCodec(mLine, payload) {
        console.log("LOG verto min js setDefaultCodec")
    ...
    function setCompat() {console.log('LOG verto min js setCompat');}
    ...
    function onStreamError(self, e) {
        console.log('LOG verto min js  There has been a problem retrieving the streams - did you allow access? Check Device Resolution', e);
        doCallback(self, "onError", e);
    }

    function onStreamSuccess(self, stream) {
        console.log("LOG verto min js Stream Success");
        doCallback(self, "onStream", stream);
    }

    function onRemoteStreamSuccess(self, stream) {
        console.log("LOG verto min js Remote Stream Success");
        doCallback(self, "onRemoteStream", stream);
    }

    function onICE(self, candidate) {
        console.log("LOG verto min js onICE");
    ...
    function doCallback(self, func, arg) {
        console.log("LOG verto min js doCallback");
    ...
    function onICEComplete(self, candidate) {
        console.log("LOG verto min js onICEComplete");
        doCallback(self, "onICEComplete");
    }

    function onChannelError(self, e) {
        console.log("LOG verto min js onChannelError");
    ...
    function onAnswerSDP(self, sdp) {
        ...
        console.log("LOG verto min js onAnswerSDP");
        doCallback(self, "onAnswerSDP", self.answer.SDP);
    }

    function onMessage(self, msg) {
        console.log("LOG verto min js onMessage");
    ...
    function onRemoteStream(self, stream) {
        console.log("LOG verto min js onRemoteStream");
        if (self.options.useVideo) {
    ...
    function onOfferSDP(self, sdp) {
        console.log("LOG verto min js onOfferSDP");
        self.mediaData.SDP = self.stereoHack(sdp.sdp);
        console.log("Offer SDP");
        doCallback(self, "onOfferSDP");
    }
    $.FSRTC.prototype.answer = function(sdp, onSuccess, onError) {
        console.log("LOG verto min js FSRTC.prototype.answer");
    ...
    $.FSRTC.prototype.stopPeer = function() {
        console.log("LOG verto min js $.FSRTC.prototype.stopPeer");
        if (self.peer) {
            self.peer.stop();
        }
    }
    $.FSRTC.prototype.stop = function() {
        console.log("LOG verto min js prototype.stop");
        var self = this;
        if (self.options.useVideo) {
            self.options.useVideo.style.display = 'none';
        ...
        if (self.localStream && !self.options.useStream) {
            ...
            console.log("LOG verto min js Stopping localStream tracks:", tracks);
        ...
        if (self.options.localVideoStream && !self.options.useStream) {
            ...
            console.log("LOG verto min js Stopping localVideoStream tracks:", tracks);
            tracks.forEach(function(track) {
                track.stop()
            })
        }
        if (self.peer) {
            console.log("LOG verto min js stopping peer");
            self.peer.stop();
        }
    };
    $.FSRTC.prototype.getMute = function() {
        console.log("LOG verto min js $.FSRTC.prototype.getMute");
        var self = this;
        return self.audioEnabled;
    }
    $.FSRTC.prototype.setMute = function(what) {
        console.log("LOG verto min js $.FSRTC.prototype.setMute");
        var self = this;
        if (!self.localStream) {
            return false;
        }
    ...
    $.FSRTC.prototype.getVideoMute = function() {
        console.log("LOG verto min js $.FSRTC.prototype.getVideoMute");
        var self = this;
        return self.videoEnabled;
    }
    $.FSRTC.prototype.setVideoMute = function(what) {
        console.log("LOG verto min js $.FSRTC.prototype.setVideoMute");
        var self = this;
        if (!self.localStream) {
            return false;
        }
    ...
    $.FSRTC.prototype.createAnswer = function(params) {
        console.log("LOG verto min js $.FSRTC.prototype.createAnswer");
        var self = this;
        self.type = "answer";
        self.remoteSDP = params.sdp;
        console.debug("LOG verto min js inbound sdp: ", params.sdp);
        ...
        function onSuccess(stream) {
            ...
            self.peer = FSRTCPeerConnection({
                ...
                onICE: function(candidate) {
                	// var var_candidate=candidate.candidate;
                	// var n = var_candidate.search(".local");
                	// console.log("n-");
                	// console.log(n);
  					// if(n == -1){
        ...
        function onError(e) {
            console.log("LOG verto min js onError");
        ...
        if (self.options.useVideo && self.options.localVideo && !self.options.useStream) {
            console.log("LOG verto min js self.options.useVideo && self.options.localVideo && !self.options.useStream");
            getUserMedia({
                constraints: {
                    audio: false,
                    video: {
                ...
                onsuccess: function(e) {
                    ...
                    console.log("LOG verto min js local video ready");
                },
                onerror: function(e) {
                    console.error("LOG verto min js local video error!", e);
                }
            });
        }
        if (self.options.useStream) {
            console.log("LOG verto min js useStream");
            if (self.options.useVideo) {
            ...
            console.log("LOG verto min js getUserMedia");
            getUserMedia({
                constraints: {
    ...
    function getMediaParams(obj) {
        console.log("LOG verto min js getMediaParams");
        var audio;
        if (obj.options.useMic && obj.options.useMic === "none") {
            console.log("LOG verto min js Microphone Disabled");
            audio = false;
        } else if (obj.options.videoParams && obj.options.screenShare) {
            console.error("LOG verto min js SCREEN SHARE", obj.options.videoParams);
            audio = false;
        } else {
            audio = {};
        ...
        if (obj.options.screenShare) {
            ...
            if (useVideo && obj.options.useCamera && obj.options.useCamera !== "none") {
                ...
                console.log("LOG verto min js Camera Disabled");
                video = false;
                useVideo = false;
            }
        }
    ...
    $.FSRTC.prototype.call = function(profile) {
        console.log("LOG verto min js $.FSRTC.prototype.call");
        checkCompat();
        var self = this;
        var screen = false;
        self.type = "offer";
        ...
        function onSuccess(stream) {
            console.log("LOG verto min js onSucces1s");
            self.localStream = stream;
            if (screen) {
                self.constraints.offerToReceiveVideo = false;
        ...
        function onError(e) {
            console.log("LOG verto min js onStreamError");
            onStreamError(self, e);
        }
        var mediaParams = getMediaParams(self);
        console.log("LOG verto min js Audio constraints", mediaParams.audio);
        console.log("LOG verto min js Video constraints", mediaParams.video);
        if (self.options.useStream) {
            console.log("LOG verto min js self.options.useStream");
            if (self.options.useVideo) {
                self.options.localVideoStream = self.options.useStream;
                if (self.options.localVideo) {
                    activateLocalVideo(self.options.localVideo, self.options.useStream);
            ...
            console.log("LOG verto min js mediaParams.audio || mediaParams.video");
            getUserMedia({
                constraints: {
                    audio: mediaParams.audio,
                    video: mediaParams.video
    ...
    function FSRTCPeerConnection(options) {
        console.log("LOG verto min js FSRTCPeerConnection");
        var gathering = false,
            done = false;
        var config = {};
        var default_ice = [{
        ...
        function ice_handler() {
            console.log("LOG verto min js ice_handler");
            done = true;
            gathering = null;
        ...
        peer.onicecandidate = function(event) {
            console.log("LOG verto min js onicecandidate");
            if (done) {
                return;
            }
            if (!gathering) {
        ...
        if (options.attachStream) {
            console.log("LOG verto min js options.attachStream");
        ...
        if (options.attachStreams && options.attachStreams.length) {
            console.log("LOG verto min js options.attachStreams && options.attachStreams.length");
        ...
        peer.ontrack = function(event) {
            console.log("LOG verto min js ontrack");
        ...
        function createOffer() {
            console.log("LOG verto min js createOffer");
        ...
        function createAnswer() {
            console.log("LOG verto min js createAnswer");
        ...
        if ((options.onChannelMessage) || !options.onChannelMessage) {
            console.log("LOG verto min js onChannelMessage");
            createOffer();
            createAnswer();
        }

        function setBandwidth(sdp) {
            console.log("LOG verto min js setBandwidth");
        ...
        function getInteropSDP(sdp) {
            console.log("LOG verto min js getInteropSDP");
        ...
        function serializeSdp(sdp) {
            console.log("LOG verto min js serializeSdp");
            return sdp;
        }
        var channel;

        function openOffererChannel() {
            console.log("LOG verto min js openOffererChannel");
        ...
        function _openOffererChannel() {
            console.log("LOG verto min js _openOffererChannel");
        ...
        function setChannelEvents() {
            console.log("LOG verto min js setChannelEvents");
        ...
        function openAnswererChannel() {
            console.log("LOG verto min js openAnswererChannel");
        ...
        function useless() {
            console.log("LOG verto min js useless");
        ...
        function onSdpError(e) {
            console.log("LOG verto min js onSdpError");
        ...
        return {
            ...
            addICE: function(candidate) {
                console.log("LOG verto min js candidate");
            ...
            sendData: function(message) {
                console.log("LOG verto min js sendData");
                if (channel) {
                    channel.send(message);
                }
            },
            stop: function() {
                console.log("LOG verto min js stop");
    ...
    function activateLocalVideo(el, stream) {
        console.log("LOG verto min js activateLocalVideo");
        el.srcObject = stream;
        el.style.display = 'block';
    }

    function deactivateLocalVideo(el) {
        console.log("LOG verto min js deactivateLocalVideo");
        el.srcObject = null;
        el.style.display = 'none';
    }

    function assureConstraintByLabel(constraint, fallbackLabel) {
        console.log("LOG verto min js assureConstraintByLabel");
    ...
    function trustyGetUserMedia(options, constraints) {
        console.log("LOG verto min js trustyGetUserMedia");
    ...
    function assignMediaIdToConstraint(mediaId, rest) {
        console.log("LOG verto min js assignMediaIdToConstraint");
    ...
    function getUserMedia(options) {
        ...
        Promise.all([assureConstraintByLabel(constraints.audio, options.useMicLabel), assureConstraintByLabel(constraints.video, options.useCameraLabel), ]).then(function(assurances) {
            ...
            console.error('Unexpected error on media id assurance attempts:', error);
        });
    }
    $.FSRTC.resSupported = function(w, h) {
        console.log("LOG verto min js resSupported");
    ...
    $.FSRTC.bestResSupported = function() {
        console.log("LOG verto min js bestResSupported");
        var w = 0,
            h = 0;
        for (var i in $.FSRTC.validRes) {
            if ($.FSRTC.validRes[i][0] >= w && $.FSRTC.validRes[i][1] >= h) {
    ...
    var checkRes = function(cam, func) {
        console.log("LOG verto min js checkRes");
    ...
    $.FSRTC.getValidRes = function(cam, func) {
        var used = [];
        console.log("LOG verto min js getValidRes");
    ...
    $.FSRTC.checkPerms = function(runtime, check_audio, check_video) {
        console.log("LOG verto min js checkPerms");
        getUserMedia({
            constraints: {
                audio: check_audio,
                video: check_video
...
(function($) {
    $.JsonRpcClient = function(options) {
        console.log("LOG verto min js JsonRpcClient");
        var self = this;
        this.options = $.extend({
            ajaxUrl: null,
            socketUrl: null,
    ...
    $.JsonRpcClient.prototype.call = function(method, params, success_cb, error_cb) {
        console.log("LOG verto min js JsonRpcClient.prototype.call");
        if (!params) {
            params = {};
        }
        ...
        if (!success_cb) {
            success_cb = function(e) {
                console.log("LOG verto min js Success: ", e);
            };
        }
        if (!error_cb) {
            error_cb = function(e) {
                console.log("LOG verto min js Error: ", e);
            };
        }
        var socket = this.options.getSocket(this.wsOnMessage);
        if (socket !== null) {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log("LOG verto min js this.options.ajaxUrl");
    ...
    $.JsonRpcClient.prototype.notify = function(method, params) {
        console.log("LOG verto min js JsonRpcClient.prototype.notify");
        if (this.options.sessid) {
            params.sessid = this.options.sessid;
        }
        var request = {
    ...
    $.JsonRpcClient.prototype.batch = function(callback, all_done_cb, error_cb) {
        console.log("LOG verto min js $.JsonRpcClient.prototype.batch");
        var batch = new $.JsonRpcClient._batchObject(this, all_done_cb, error_cb);
        callback(batch);
        batch._execute();
    };
    $.JsonRpcClient.prototype.socketReady = function() {
        console.log("LOG verto min js $.JsonRpcClient.prototype.socketReady");
    ...
    $.JsonRpcClient.prototype.closeSocket = function() {
        console.log("LOG verto min js $.JsonRpcClient.prototype.closeSocket");
        var self = this;
        if (self.socketReady()) {
    ...
    $.JsonRpcClient.prototype.loginData = function(params) {
        console.log("LOG verto min js $.JsonRpcClient.prototype.loginData");
        var self = this;
    ...
    $.JsonRpcClient.prototype.connectSocket = function(onmessage_cb) {
        console.log("LOG verto min js $.JsonRpcClient.prototype.connectSocket");
        var self = this;
        if (self.to) {
            ...
            if (self._ws_socket) {
                ...
                self._ws_socket.onclose = function(w) {
                    ...
                    self.to = setTimeout(function() {
                        console.log("LOG verto min js Attempting Reconnection....");
    ...
    $.JsonRpcClient.prototype.stopRetrying = function() {
        console.log("LOG verto min js $.JsonRpcClient.prototype.stopRetrying");
        if (self.to)
            clearTimeout(self.to);
    }
    $.JsonRpcClient.prototype._getSocket = function(onmessage_cb) {
        console.log("LOG verto min js $.JsonRpcClient.prototype._getSocket");
    ...
    $.JsonRpcClient.prototype._wsCall = function(socket, request, success_cb, error_cb) {
        console.log("LOG verto min js $.JsonRpcClient.prototype._wsCall");
    ...
    $.JsonRpcClient.prototype._wsOnMessage = function(event) {
        console.log("LOG verto min js $.JsonRpcClient.prototype._wsOnMessage");
        var response;
        ...
        try {
                ...
                if ('result' in response && this._ws_callbacks[response.id]) {
                    ...
                    if (!self.authing && response.error.code == -32000 && self.options.login && self.options.passwd) {
                        ...
                        this.call("login", {
                            ...
                            self.authing = false;
                            console.log("LOG verto min js logged in");
                            ...
                            self.authing = false;
                            console.log("LOG verto min js logged in, resending request id: " + response.id);
                            ...
                            console.log("LOG verto min js error logging in, request id:", response.id);
                            delete self._ws_callbacks[response.id];
                            error_cb(response.error, this);
                            if (self.options.onWSLogin) {
            ...
            console.log("LOG verto min js ERROR: " + err);
            return;
        }
        if (typeof this.options.onmessage === 'function') {
            console.log("LOG verto min js typeof this.options.onmessage");
    ...
    $.JsonRpcClient._batchObject = function(jsonrpcclient, all_done_cb, error_cb) {
        console.log("LOG verto min js typeof $.JsonRpcClient._batchObject ");
        this._requests = [];
    ...
    $.JsonRpcClient._batchObject.prototype.call = function(method, params, success_cb, error_cb) {
        console.log("LOG verto min js $.JsonRpcClient._batchObject.prototype.call ");
        if (!params) {
            params = {};
        }
        if (this.options.sessid) {
    ...
    $.JsonRpcClient._batchObject.prototype.notify = function(method, params) {
        console.log("LOG verto min js $.JsonRpcClient._batchObject.prototype.notify");
        if (this.options.sessid) {
            params.sessid = this.options.sessid;
        }
        this._requests.push({
    ...
    $.JsonRpcClient._batchObject.prototype._execute = function() {
        console.log("LOG verto min js $.JsonRpcClient._batchObject.prototype._execute");
        var self = this;
        ...
        success_cb = function(data) {
            console.log("LOG verto min js success_cb");
            self._batchCb(data, handlers, self.all_done_cb);
        };
        if (self.jsonrpcclient.options.ajaxUrl === null) {
            console.log("LOG verto min js self.jsonrpcclient.options.ajaxUrl");
    ...
    $.JsonRpcClient._batchObject.prototype._batchCb = function(result, handlers, all_done_cb) {
        console.log("LOG verto min js $.JsonRpcClient._batchObject.prototype._batchCb");
...
(function($) {
    ...
    var generateGUID = (typeof(window.crypto) !== 'undefined' && typeof(window.crypto.getRandomValues) !== 'undefined') ? function() {
        console.log("LOG verto min js generateGUID = (typeof(window.crypto)");
    ...
    $.verto = function(options, callbacks) {
        console.log("LOG verto min js $.verto = function(options, callbacks)");
        var verto = this;
        ...
        if (verto.options.ringFile && verto.options.tag) {
            verto.ringer = $("#" + tag);
            console.log(verto.ringer);
        }
        verto.rpcClient.call('login', {});
    };
    $.verto.prototype.deviceParams = function(obj) {
        console.log("LOG verto min js $.verto.prototype.deviceParams");
        var verto = this;
        for (var i in obj) {
    ...
    $.verto.prototype.videoParams = function(obj) {
        console.log("LOG verto min js $.verto.prototype.videoParams");
        var verto = this;
        for (var i in obj) {
    ...
    $.verto.prototype.loginData = function(params) {
        console.log("LOG verto min js $.verto.prototype.loginData");
        var verto = this;
    ...
    $.verto.prototype.logout = function(msg) {
        console.log("LOG verto min js $.verto.prototype.logout");
        var verto = this;
    ...
    $.verto.prototype.login = function(msg) {
        console.log("LOG verto min js $.verto.prototype.login");
        var verto = this;
        verto.logout();
        verto.rpcClient.call('login', {});
    };
    $.verto.prototype.message = function(msg) {
        console.log("LOG verto min js $.verto.prototype.message ");
        var verto = this;
        var err = 0;
        if (!msg.to) {
    ...
    $.verto.prototype.processReply = function(method, success, e) {
        console.log("LOG verto min js $.verto.prototype.processReply ");
        var verto = this;
        var i;
        switch (method) {
    ...
    $.verto.prototype.sendMethod = function(method, params) {
        console.log("LOG verto min js $.verto.prototype.sendMethod ");
        var verto = this;
    ...
    function do_subscribe(verto, channel, subChannels, sparams) {
        console.log("LOG verto min js function do_subscribe");
        var params = sparams || {};
    ...
    $.verto.prototype.subscribe = function(channel, sparams) {
        console.log("LOG verto min js $.verto.prototype.subscribe");
        var verto = this;
        var r = [];
        var subChannels = [];
        var params = sparams || {};
    ...
    $.verto.prototype.unsubscribe = function(handle) {
        console.log("LOG verto min js $.verto.prototype.unsubscribe");
        var verto = this;
        var i;
        if (!handle) {
    ...
    $.verto.prototype.broadcast = function(channel, params) {
        console.log("LOG verto min js $.verto.prototype.broadcast");
        var verto = this;
        var msg = {
            eventChannel: channel,
            data: {}
    ...
    $.verto.prototype.purge = function(callID) {
        console.log("LOG verto min js $.verto.prototype.purge");
        var verto = this;
        var x = 0;
        var i;
    ...
    $.verto.prototype.hangup = function(callID) {
        console.log("LOG verto min js $.verto.prototype.hangup");
        var verto = this;
        if (callID) {
    ...
    $.verto.prototype.newCall = function(args, callbacks) {
        console.log("LOG verto min js $.verto.prototype.newCall ");
        var verto = this;
    ...
    $.verto.prototype.handleMessage = function(data) {
        console.log("LOG verto min js $.verto.prototype.handleMessage ");
        var verto = this;
        if (!(data && data.method)) {
    ...
    var del_array = function(array, name) {
        console.log("LOG verto min js del_array");
        var r = [];
    ...
    var hashArray = function() {
        console.log("LOG verto min js hashArray");
        var vha = this;
        var hash = {};
        var array = [];
        ...
        vha.clear = function() {
            console.log("LOG verto min js clear");
            hash = undefined;
            array = undefined;
            hash = {};
            array = [];
        ...
        vha.add = function(name, val, insertAt) {
            console.log("LOG verto min js add");
            var redraw = false;
            if (!hash[name]) {
        ...
        vha.del = function(name) {
            console.log("LOG verto min js vha.del");
            var r = false;
            if (hash[name]) {
        ...
        vha.get = function(name) {
            console.log("LOG verto min js vha.get");
            return hash[name];
        };
        vha.order = function() {
            console.log("LOG verto min js vha.order");
            return array;
        };
        vha.hash = function() {
            console.log("LOG verto min js vha.hash");
            return hash;
        };
        vha.indexOf = function(name) {
            console.log("LOG verto min js vha.indexof");
            var len = array.length;
            for (var i = 0; i < len; i++) {
                if (array[i] == name) {
                    return i;
        ...
        vha.arrayLen = function() {
            console.log("LOG verto min js arrayLen.del");
            return array.length;
        };
        vha.asArray = function() {
            console.log("LOG verto min js asArray");
            var r = [];
            var len = array.length;
            for (var i = 0; i < len; i++) {
                var key = array[i];
        ...
        vha.each = function(cb) {
            console.log("LOG verto min js vha.each");
            var len = array.length;
            for (var i = 0; i < len; i++) {
        ...
        vha.dump = function(html) {
            console.log("LOG verto min js vha.dump ");
            var str = "";
    ...
    $.verto.liveArray = function(verto, context, name, config) {
        console.log("LOG verto min js $.verto.liveArray ");
        var la = this;
        var lastSerno = 0;
        var binding = null;
        ...
        la.errs = 0;
        la.clear = function() {
            console.log("LOG verto min js la.clear ");
            la._clear();
            lastSerno = 0;
            if (la.onChange) {
                la.onChange(la, {
        ...
        la.checkSerno = function(serno) {
            console.log("LOG verto min js la.checkserno ");
            if (serno < 0) {
                return true;
            }
        ...
        la.reorder = function(serno, a) {
            console.log("LOG verto min js la.reorder ");
        ...
        la.init = function(serno, val, key, index) {
            console.log("LOG verto min js la.init ");
        ...
        la.bootObj = function(serno, val) {
            console.log("LOG verto min js la.bootobj ");
            if (la.checkSerno(serno)) {
                for (var i in val) {
        ...
        la.add = function(serno, val, key, index) {
            console.log("LOG verto min js la.add ");
            if (key === null || key === undefined) {
                key = serno;
            }
            if (la.checkSerno(serno)) {
        ...
        la.modify = function(serno, val, key, index) {
            console.log("LOG verto min js la.modify ");
            if (key === null || key === undefined) {
                key = serno;
            }
            if (la.checkSerno(serno)) {
        ...
        la.del = function(serno, key, index) {
            console.log("LOG verto min js la.del ");
            if (key === null || key === undefined) {
                key = serno;
            }
            if (la.checkSerno(serno)) {
        ...
        var eventHandler = function(v, e, la) {
            console.log("LOG verto min js la.eventHandler ");
            var packet = e.data;
        ...
        if (la.context) {
            console.log("LOG verto min js la.context ");
        ...
        la.destroy = function() {
            console.log("LOG verto min js la.destroy ");
            la._clear();
            la.verto.unsubscribe(binding);
        };
        la.sendCommand = function(cmd, obj) {
            console.log("LOG verto min js la.sendCommand ");
            var self = la;
        ...
        la.bootstrap = function(obj) {
            console.log("LOG verto min js la.bootstrap ");
            var self = la;
            la.sendCommand("bootstrap", obj);
        };
        la.changepage = function(obj) {
            console.log("LOG verto min js la.changepage ");
            var self = la;
            self.clear();
            self.broadcast(self.context, {
                liveArray: {
        ...
        la.heartbeat = function(obj) {
            console.log("LOG verto min js la.heartbeat ");
            var self = la;
    ...
    $.verto.liveTable = function(verto, context, name, jq, config) {
        console.log("LOG verto min js la.liveTable ");
        var dt;
        ...
        la.onErr = function(obj, args) {
            console.log("LOG verto min js OnERr ");
            console.error("Error: ", obj, args);
        };

        function genRow(data) {
            console.log("LOG verto min js genRow ");
        ...
        function genArray(obj) {
            console.log("LOG verto min js genArray ");
        ...
        la.onChange = function(obj, args) {
            console.log("LOG verto min js onChange ");
            var index = 0;
            var iserr = 0;
            if (!dt) {
                if (!config.aoColumns) {
    ...
    $.verto.conf = function(verto, params) {
        console.log("LOG verto min js $.verto.conf ");
        var conf = this;
        conf.params = $.extend({
            dialog: null,
            hasVid: false,
    ...
    $.verto.conf.prototype.modCommand = function(cmd, id, value) {
        console.log("LOG verto min js verto.conf.prototype.modCommand ");
        var conf = this;
    ...
    $.verto.conf.prototype.destroy = function() {
        console.log("LOG verto min js verto.conf.prototype.destroy ");
        var conf = this;
        conf.destroyed = true;
    ...
    function createMainModeratorMethods() {
        console.log("LOG verto min js createMainModeratorMethods ");
    ...
    $.verto.confMan = function(verto, params) {
        console.log("LOG verto min js $.verto.confMan ");
        var confMan = this;
        ...
        function genMainMod(jq) {
            console.log("LOG verto min js $.genMainMod ");
        ...
        function genControls(jq, rowid) {
            console.log("LOG verto min js genControls ");
        ...
        verto.subscribe(confMan.params.laData.infoChannel, {
            handler: function(v, e) {
                console.log("LOG verto min js subscribe handler");
        ...
        verto.subscribe(confMan.params.laData.chatChannel, {
            handler: function(v, e) {
                console.log("LOG verto min js subscribe handler1");
        ...
        if (confMan.params.laData.role === "moderator") {
            console.log("LOG verto min js confMan.params.laData.role");
            atitle = "Action";
            awidth = 600;
    ...
    $.verto.confMan.prototype.modCommand = function(cmd, id, value) {
        console.log("LOG verto min js $.verto.confMan.prototype.modCommand ");
        var confMan = this;
    ...
    $.verto.confMan.prototype.sendChat = function(message, type) {
        console.log("LOG verto min js $.verto.confMan.prototype.sendChat ");
        var confMan = this;
        confMan.verto.rpcClient.call("verto.broadcast", {
    ...
    $.verto.confMan.prototype.destroy = function() {
        console.log("LOG verto min js $.verto.confMan.prototype.destroy ");
        var confMan = this;
    ...
    $.verto.dialog = function(direction, verto, params) {
        console.log("LOG verto min js $.$.verto.dialog ");
        var dialog = this;
        ...
        RTCcallbacks.onICESDP = function(rtc) {
            console.log("LOG verto min js $.RTCcallbacks.onICESDP");
        	console.log("rtc-");
        	console.log(rtc);
        	console.log(typeof rtc);
        	console.log(typeof rtc.mediaData.SDP);
        	console.log(rtc.mediaData.SDP);
        	result=indexes(rtc.mediaData.SDP, "a=candidate:");
        	console.log(result);
        	var x=rtc.mediaData.SDP;
        	candidate_val=x.substring(result[0], result[1]);
        	console.log(candidate_val);
        	var n = candidate_val.search(".local");
        	if(n!=-1){
        		var new_mediaData_val1=x.substring(0,result[0]-1);
        		console.log(new_mediaData_val1);
        		var new_mediaData_val2=x.substring(result[1],x.length);
        		console.log(new_mediaData_val2);
        		new_mediaData_val=new_mediaData_val1+'\n'+new_mediaData_val2;
        		console.log(new_mediaData_val);
        	}
        	else
        	{
        		new_mediaData_val=rtc.mediaData.SDP;
        	}
            ...
            if (rtc.type == "offer") {
            	...
            	console.log("x3-");
            	console.log(dialog.rtc.mediaData.SDP);
        ...
        RTCcallbacks.onICE = function(rtc) {
            console.log("LOG verto min js RTCcallbacks.onICE");
            if (rtc.type == "offer") {
        ...
        RTCcallbacks.onStream = function(rtc, stream) {
            console.log("LOG verto min js RTCcallbacks.onStream");
        ...
        RTCcallbacks.onRemoteStream = function(rtc, stream) {
            console.log("LOG verto min js RTCcallbacks.onRemoteStream ");
        ...
        RTCcallbacks.onError = function(e) {
            console.log("LOG verto min js RTCcallbacks.onError  ");
    ...
    $.verto.dialog.prototype.invite = function() {
        console.log("LOG verto min js $.verto.dialog.prototype.invite  ");
        var dialog = this;
        dialog.rtc.call();
    };
    $.verto.dialog.prototype.sendMethod = function(method, obj) {
        console.log("LOG verto min js $.verto.dialog.prototype.sendMethod");
        var dialog = this;
        obj.dialogParams = {};
    ...
    function checkStateChange(oldS, newS) {
        console.log("LOG verto min js checkStateChange");
    ...
    function find_name(id) {
        console.log("LOG verto min js find_name(id)");
    ...
    $.verto.dialog.prototype.setAudioPlaybackDevice = function(sinkId, callback, arg) {
        console.log("LOG verto min js $.verto.dialog.prototype.setAudioPlaybackDevice");
        var dialog = this;
    ...
    $.verto.dialog.prototype.setState = function(state) {
        console.log("LOG verto min js $.verto.dialog.prototype.setState");
        var dialog = this;
    ...
    $.verto.dialog.prototype.processReply = function(method, success, e) {
        console.log("LOG verto min js $.verto.dialog.prototype.processReply");
        var dialog = this;
        switch (method) {
            case "verto.answer":
            case "verto.attach":
    ...
    $.verto.dialog.prototype.hangup = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.hangup");
        var dialog = this;
        if (params) {
            if (params.causeCode) {
    ...
    $.verto.dialog.prototype.stopRinging = function() {
        console.log("LOG verto min js $.verto.dialog.prototype.stopRinging");
        var dialog = this;
    ...
    $.verto.dialog.prototype.indicateRing = function() {
        console.log("LOG verto min js $.verto.dialog.prototype.indicateRing");
        var dialog = this;
        if (dialog.verto.ringer) {
        	console.log("dialog.verto.ringer-");
    ...
    $.verto.dialog.prototype.ring = function() {
        console.log("LOG verto min js $.verto.dialog.prototype.ring");
        var dialog = this;
        dialog.setState($.verto.enum.state.ringing);
        dialog.indicateRing();
    };
    $.verto.dialog.prototype.useVideo = function(on) {
        console.log("LOG verto min js $.verto.dialog.prototype.useVideo");
        var dialog = this;
    ...
    $.verto.dialog.prototype.rtt = function(obj) {
        console.log("LOG verto min js $.verto.dialog.prototype.rtt");
        var dialog = this;
        var pobj = {};
        if (!obj) {
            return false;
    ...
    $.verto.dialog.prototype.replace = function(replaceCallID, params) {
        console.log("LOG verto min js $.verto.dialog.prototype.replace");
        var dialog = this;
        if (replaceCallID) {
            dialog.sendMethod("verto.modify", {
                action: "replace",
    ...
    $.verto.dialog.prototype.hold = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.hold");
        var dialog = this;
        dialog.sendMethod("verto.modify", {
            action: "hold",
            params: params
    ...
    $.verto.dialog.prototype.unhold = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.unhold");
        var dialog = this;
        dialog.sendMethod("verto.modify", {
            action: "unhold",
            params: params
    ...
    $.verto.dialog.prototype.toggleHold = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.toggleHold");
        var dialog = this;
        dialog.sendMethod("verto.modify", {
            action: "toggleHold",
            params: params
    ...
    $.verto.dialog.prototype.message = function(msg) {
        console.log("LOG verto min js $.verto.dialog.prototype.message");
        var dialog = this;
        var err = 0;
    ...
    $.verto.dialog.prototype.answer = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.answer ");
        var dialog = this;
        if (!dialog.answered) {
            if (!params) {
                params = {};
    ...
    $.verto.dialog.prototype.handleAnswer = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.handleAnswer ");
        var dialog = this;
    ...
    $.verto.dialog.prototype.cidString = function(enc) {
        console.log("LOG verto min js $.verto.dialog.prototype.cidString ");
        var dialog = this;
    ...
    $.verto.dialog.prototype.sendMessage = function(msg, params) {
        console.log("LOG verto min js $.verto.dialog.prototype.sendMessage ");
        var dialog = this;
    ...
    $.verto.dialog.prototype.handleInfo = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.handleInfo ");
        var dialog = this;
        dialog.sendMessage($.verto.enum.message.info, params);
    };
    $.verto.dialog.prototype.handleDisplay = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.handleDisplay ");
        var dialog = this;
    ...
    $.verto.dialog.prototype.handleMedia = function(params) {
        console.log("LOG verto min js $.verto.dialog.prototype.handleMedia ");
        var dialog = this;
        if (dialog.state.val >= $.verto.enum.state.early.val) {
            return;
        }
    ...
    $(window).bind(unloadEventName, function() {
        console.log("LOG verto min js unloadEventName ");
    ...
    var checkDevices = function(runtime) {
        ...
        function gotDevices(deviceInfos) {
            console.log("LOG verto min js gotDevices ");
    ...
    $.verto.init = function(obj, runtime) {
        console.log("LOG verto min js $.verto.init  ");
        if (!obj) {
            obj = {};
        }
...
(function(f) {
    console.log("LOG verto min js function  ");
    ...
    console.log("LOG verto min js function2  ");
    ...
    return (function() {
        ...
        2: [function(require, module, exports) {
            ...
            function _interopRequireWildcard(obj) {
                console.log("LOG verto min js _interopRequireWildcard  ");
            ...
            function adapterFactory() {
                console.log("LOG verto min js adapterFactory  ");
        ...
        3: [function(require, module, exports) {
        	console.log("LOG verto min js exports  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
            ...
            function walkStats(stats, base, resultSet) {
                console.log("LOG verto min js walkStats  ");
            ...
            function filterStats(result, track, outbound) {
                console.log("LOG verto min js filterStats  ");
            ...
            function shimMediaStream(window) {
                console.log("LOG verto min js shimMediaStream  ");
                window.MediaStream = window.MediaStream || window.webkitMediaStream;
            }

            function shimOnTrack(window) {
                console.log("LOG verto min js shimOnTrack  ");
            ...
            function shimGetSendersWithDtmf(window) {
                console.log("LOG verto min js shimGetSendersWithDtmf  ");
            ...
            function shimSenderReceiverGetStats(window) {
                console.log("LOG verto min js shimSenderReceiverGetStats  ");
            ...
            function shimAddTrackRemoveTrackWithNative(window) {
                console.log("LOG verto min js shimAddTrackRemoveTrackWithNative  ");
            ...
            function shimAddTrackRemoveTrack(window) {
                console.log("LOG verto min js shimAddTrackRemoveTrack  ");
                ...
                function replaceInternalStreamId(pc, description) {
                    console.log("LOG verto min js replaceInternalStreamId  ");
                ...
                function replaceExternalStreamId(pc, description) {
                    console.log("LOG verto min js replaceExternalStreamId  ");
                    var sdp = description.sdp;
                    Object.keys(pc._reverseStreams || []).forEach(function(internalId) {
                        var externalStream = pc._reverseStreams[internalId];
                        var internalStream = pc._streams[externalStream.id];
                ...
                window.RTCPeerConnection.prototype.setLocalDescription = function() {
                    console.log("LOG verto min js setLocalDescription  ");
                ...
                window.RTCPeerConnection.prototype.removeTrack = function(sender) {
                    console.log("LOG verto min js removeTrack  ");
                    var _this14 = this;
                    if (this.signalingState === 'closed') {
                        throw new DOMException('The RTCPeerConnection\'s signalingState is \'closed\'.', 'InvalidStateError');
                    }
            ...
            function shimPeerConnection(window) {
                console.log("LOG verto min js shimPeerConnection  ");
                ...
                window.RTCPeerConnection.prototype.getStats = function(selector, successCallback, errorCallback) {
                    ...
                    var fixChromeStats_ = function fixChromeStats_(response) {
                    	console.log("fixChromeStats_candidate");
                        var standardReport = {};
                ...
                ['setLocalDescription', 'setRemoteDescription', 'addIceCandidate'].forEach(function(method) {
                    console.log("LOG verto min js setRemoteDescription  ");
                    var nativeMethod = window.RTCPeerConnection.prototype[method];
                    window.RTCPeerConnection.prototype[method] = function() {
                ...
                window.RTCPeerConnection.prototype.addIceCandidate = function() {
                    console.log("LOG verto min js addIceCandidate  ");
                    if (!arguments[0]) {
                        if (arguments[1]) {
            ...
            function fixNegotiationNeeded(window) {
                console.log("LOG verto min js fixNegotiationNeeded  ");
        ...
        4: [function(require, module, exports) {
            ...
            function shimGetDisplayMedia(window, getSourceId) {
                console.log("LOG verto min js shimGetDisplayMedia  ");
                ...
                window.navigator.mediaDevices.getDisplayMedia = function(constraints) {
                    console.log("LOG verto min js getDisplayMedia  ");
        ...
        5: [function(require, module, exports) {
            console.log("LOG verto min js require1  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
            ...
            function shimGetUserMedia(window) {
                console.log("LOG verto min js shimGetUserMedia  ");
        ...
        6: [function(require, module, exports) {
            ...
            function _interopRequireDefault(obj) {
                console.log("LOG verto min js _interopRequireDefault  ");
            ...
            function shimRTCIceCandidate(window) {
                console.log("LOG verto min js shimRTCIceCandidate  ");
                ...
                utils.wrapPeerConnectionEvent(window, 'icecandidate', function(e) {
                	console.log("wrapPeerConnectionEvent icecandidate");
                    if (e.candidate) {
            ...
            function shimMaxMessageSize(window) {
                console.log("LOG verto min js shimMaxMessageSize  ");
            ...
            function shimSendThrowTypeError(window) {
                console.log("LOG verto min js shimSendThrowTypeError  ");
            ...
            function shimConnectionState(window) {
                console.log("LOG verto min js shimConnectionState  ");
            ...
            function removeAllowExtmapMixed(window) {
                console.log("LOG verto min js removeAllowExtmapMixed  ");
                if (!window.RTCPeerConnection) {
                    return;
                }
                var browserDetails = utils.detectBrowser(window);
        ...
        7: [function(require, module, exports) {
            console.log("LOG verto min js require3  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
        ...
        8: [function(require, module, exports) {
            console.log("LOG verto min js require4  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
            ...
            function filterIceServers(iceServers, edgeVersion) {
                console.log("LOG verto min js filterIceServers  ");
                var hasTurn = false;
        ...
        9: [function(require, module, exports) {
            'use strict';
            console.log("LOG verto min js exports3  ");
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
            exports.shimGetDisplayMedia = shimGetDisplayMedia;
        ...
        11: [function(require, module, exports) {
            console.log("LOG verto min js require5  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
        ...
        12: [function(require, module, exports) {
            console.log("LOG verto min js require6  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
        ...
        13: [function(require, module, exports) {
            console.log("LOG verto min js require7 ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
        ...
        14: [function(require, module, exports) {
        	console.log("LOG verto min js require8  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
            ...
            function shimCallbacksAPI(window) {
                ...
                withCallback = function withCallback(candidate, successCallback, failureCallback) {
                	console.log("function withCallback(candidate");
            ...
            function shimGetUserMedia(window) {
            	console.log("shimGetUserMedia88");
                var navigator = window && window.navigator;
            ...
            function shimTrackEventTransceiver(window) {
                if ((typeof window === 'undefined' ? 'undefined' : _typeof(window)) === 'object' && window.RTCPeerConnection && 'receiver' in window.RTCTrackEvent.prototype && !window.RTCTransceiver) {
                    Object.defineProperty(window.RTCTrackEvent.prototype, 'transceiver', {
                        get: function get() {
                            return {
                                receiver: this.receiver
        ...
        15: [function(require, module, exports) {
            console.log("LOG verto min js require9  ");
            'use strict';
            Object.defineProperty(exports, "__esModule", {
                value: true
            });
        ...
        16: [function(require, module, exports) {
            console.log("LOG verto min js require10  ");
            'use strict';
            ...
            function getCommonCapabilities(localCapabilities, remoteCapabilities) {
            	console.log("getCommonCapabilities");
            ...
            module.exports = function(window, edgeVersion) {
                ...
                RTCPeerConnection.prototype._disposeIceAndDtlsTransports = function(sdpMLineIndex) {
                	console.log("_disposeIceAndDtlsTransports");
                    var iceGatherer = this.transceivers[sdpMLineIndex].iceGatherer;
                    if (iceGatherer) {
                    ...
                    sections.forEach(function(mediaSection, sdpMLineIndex) {
                        ...
                        if (description.type === 'offer' && !rejected) {
                            ...
                            var hasRtx = commonCapabilities.codecs.filter(function(c) {
                            	console.log("commonCapabilities.codecs.filter")
                ...
                RTCPeerConnection.prototype.createOffer = function() {
                    ...
                    pc.transceivers.forEach(function(transceiver, sdpMLineIndex) {
                        ...
                        if (edgeVersion < 15019) {
                        	console.log("edgeVersion < 15019")
                            localCapabilities.codecs = localCapabilities.codecs.filter(function(codec) {
                                return codec.name !== 'rtx';
                            });
                        }
                        localCapabilities.codecs.forEach(function(codec) {
                        	console.log("localCapabilities.codecs.forEach");
                ...
                RTCPeerConnection.prototype.createAnswer = function() {
                    ...
                    pc.transceivers.forEach(function(transceiver, sdpMLineIndex) {
                        ...
                        var commonCapabilities = getCommonCapabilities(transceiver.localCapabilities, transceiver.remoteCapabilities);
                        var hasRtx = commonCapabilities.codecs.filter(function(c) {
                        	console.log("hasRtx = commonCapabilities.codecs.filter");
                            return c.name.toLowerCase() === 'rtx';
                        }).length;
                        if (!hasRtx && transceiver.sendEncodingParameters[0].rtx) {
                            delete transceiver.sendEncodingParameters[0].rtx;
        ...
        17: [function(require, module, exports) {
            console.log("LOG verto min js require11  ");
            'use strict';
            var SDPUtils = {};
            ...
            SDPUtils.writeRtpMap = function(codec) {
            	console.log("SDPUtils.writeRtpMap = function(codec)");
            ...
            SDPUtils.writeFmtp = function(codec) {
            	console.log("SDPUtils.writeFmtp = function(codec)")
                var line = '';
                var pt = codec.payloadType;
                if (codec.preferredPayloadType !== undefined) {
                    pt = codec.preferredPayloadType;
            ...
            SDPUtils.writeRtcpFb = function(codec) {
            	console.log("SDPUtils.writeRtcpFb = function(codec)");
                var lines = '';
                var pt = codec.payloadType;
                if (codec.preferredPayloadType !== undefined) {
                    pt = codec.preferredPayloadType;
            ...
            SDPUtils.parseRtpParameters = function(mediaSection) {
            	console.log("SDPUtils.parseRtpParameters = function(mediaSection)");
                var description = {
                    codecs: [],
                    headerExtensions: [],
                    fecMechanisms: [],
            ...
            SDPUtils.writeRtpDescription = function(kind, caps) {
            	console.log("SDPUtils.writeRtpDescription = function(kind, caps)");
                var sdp = '';
                sdp += 'm=' + kind + ' ';
            ...
            SDPUtils.parseRtpEncodingParameters = function(mediaSection) {
                ...
                description.codecs.forEach(function(codec) {
                	console.log("description.codecs.forEach(function(codec)");
...
function indexes(source, find) {
  var result = [];
  console.log("LOG verto min js indexes  ");
...
;(function($, window) {
    ...
    // ── Helper: fire an event handler safely ──────────────────────────────────
    function _fire(name) {
        var args = Array.prototype.slice.call(arguments, 1);
        if (typeof _handlers[name] === 'function') {
            try { _handlers[name].apply(null, args); } catch(e) {
                console.error('VertoPhone handler error [' + name + ']:', e);
            }
        }
    }

    // ── Helper: find candidate indexes in SDP ─────────────────────────────────
    function _indexes(source, find) {
        var result = [];
    ...
    var _callbacks = {
        ...
        onDialogState: function(d) {
            ...
            switch (d.state) {
                ...
                case $.verto.enum.state.ringing:
                    var cid     = d.cidString();
                    var idx     = cid.search('<');
                    var callerName   = idx !== -1 ? cid.substring(0, idx - 1)                      : cid;
                    var callerNumber = idx !== -1 ? cid.substring(idx + 1, cid.length - 1) : cid;
                    _fire('onIncomingCall', callerName, callerNumber, d);
                    break;

                case $.verto.enum.state.trying:
                    _cause = 'trying';
                    _fire('onCallTrying', d.params.destination_number || '');
                    break;
                ...
                case $.verto.enum.state.active:
                    _cause = 'active';
                    var num = d.params.remote_caller_id_number ||
                              d.params.destination_number || '';
                    _fire('onCallActive', num, d);
                    break;

                case $.verto.enum.state.hangup:
                    _cause = 'hangup';
                    _fire('onCallHangup', d.cause || 'NORMAL_CLEARING');
                    break;
        ...
        onMessage: function(v, dialog, msg, data) {
            if (msg === $.verto.enum.message.display && dialog) {
                _fire('onCallActive',
                    dialog.params.remote_caller_id_number || '', dialog);
            }
        },

        onEvent: function(v, e) {
            /* optional: bubble raw events */
            _fire('onEvent', e);
        }
    };

    // ── Internal: create/connect $.verto instance ────────────────────────────
    function _connect(cfg) {
        _config = cfg;
        if (_verto) {
            try { _verto.logout(); } catch(e) {}
            _verto = null;
        }
        _verto = new $.verto({
            login      : cfg.login,
            ...
            tag        : cfg.tag        || 'webcam',
            ringFile   : cfg.ringFile   || '',
            videoParams: cfg.videoParams || { minWidth:'1280', minHeight:'720', minFrameRate:30 },
            audioParams: cfg.audioParams || {
                googAutoGainControl : false,
                googNoiseSuppression: false,
                googHighpassFilter  : false
            },
            iceServers : (cfg.useStun !== undefined) ? cfg.useStun : true
        }, _callbacks);
        _verto.login();
    }
    ...
    window.VertoPhone = {
        ...
        /**
         * Set custom endpoints before calling login()
         * @param {Object} options - { provisioningUrl, socketUrl }
         */
        configure: function(options) {
            if (options.socketUrl) this._socketUrl = options.socketUrl;
        },

        /**
         * Provision credentials (NO API VERSION)
         * @param {Object}   params
         * @param {string}   params.agentId
         * @param {string}   params.domain
         * @param {string}   params.password
         * @param {string}   [params.displayName]
         * @param {Array}    [params.statusList]
         * @param {function} onStatusList(statuses)
         *        — called with array of { status_text, status_number, state_enum }
         */
        login: function(params, onStatusList) {
            ...
            if (!agentId || !password || !domain) {
                alert('agentId, domain and password are required');
                return;
            }

            // Handle status list (previously from provisioning API)
            if (statusList && Array.isArray(statusList)) {
                _stateNames = statusList;
                _stateCodes = statusList.map(s => s.status_number);

                for (let s of statusList) {
                    if (s.status_text === 'Logout') {
                        _logoutCode = s.status_number;
                    }
                }

                if (typeof onStatusList === 'function') {
                    onStatusList(statusList, displayName, agentId);
                }
            }

            // Direct connect (NO API CALL)
            _connect({
                login: agentId + '@' + domain,
                passwd: password,
                socketUrl: socketUrl || this._socketUrl,
                ...
                callerName: displayName || agentId,
                callerNumber: agentId
            });
        },

        /**
         * Low-level connect — supply config directly (no provisioning API).
         */
        connect: function(cfg, handlers) {
            if (handlers) this.setHandlers(handlers);
            _connect(cfg);
        },

        /** Register event handlers (can be called at any time) */
        ...
        makeCall: function(number) {
            if (!_verto || !number) return null;
            _redialNum = number;
            _cur_call = _verto.newCall({
                destination_number: number,
                caller_id_name    : _config.callerName   || '',
                caller_id_number  : _config.callerNumber || '',
                useVideo  : false,
                useStereo : false
            });
            return _cur_call;
        ...
        /** Make a call with full options */
        makeCallFull: function(opts) {
            if (!_verto) return null;
            if (opts.destination_number) _redialNum = opts.destination_number;
            _cur_call = _verto.newCall(opts);
            return _cur_call;
        },

        /** Redial last number */
        redial: function() {
            if (_redialNum) return this.makeCall(_redialNum);
        },

        /** Answer active incoming call */
        answer: function(opts) {
            if (!_cur_call) return;
            _cur_call.answer($.extend({
                useStereo         : false,
                callee_id_name    : _config.callerName   || '',
                callee_id_number  : _config.callerNumber || ''
            }, opts || {}));
        },
        ...
        reject: function() {
            if (!_cur_call) return;
            _cur_call.hangup({ cause: 'CALL_REJECTED' });
        },

        /** Hang up active call */
        hangup: function() {
            if (_verto) _verto.hangup();
            _cur_call = null;
        },

        /** Toggle hold */
        hold: function() {
            if (_cur_call) _cur_call.toggleHold();
        },

        /** Blind transfer */
        transfer: function(number) {
            if (_cur_call && number) _cur_call.transfer(number);
        },

        /** Send DTMF digit */
        dtmf: function(digit) {
            if (_cur_call) _cur_call.dtmf(digit);
        },

        /**
         * Change agent state by dialing a feature code.
         * statusNumber is the numeric code e.g. "*84"
         */
        changeState: function(statusNumber) {
            if (!_verto || !statusNumber) return;
            var tmp = _verto.newCall({
                destination_number: statusNumber,
                caller_id_name    : _config.callerName   || '',
                caller_id_number  : _config.callerNumber || '',
                useVideo  : false,
                useStereo : false
            });
            // auto-hangup feature codes after 3s
            if (tmp) {
                setTimeout(function() {
                    try { tmp.hangup(); } catch(e) {}
                }, 3000);
            }
        },

        /** Logout: dial logout code then disconnect */
        logout: function() {
            var self = this;
            if (_logoutCode) self.changeState(_logoutCode);
            setTimeout(function() {
                if (_verto) {
                    try { _verto.logout(); } catch(e) {}
                    _verto = null;
        ...
        /** Is WebSocket connected and logged in? */
        isConnected: function() { return _online; },

        /** Get current active call dialog */
        getActiveCall: function() { return _cur_call; },

        /** Get status list (populated after login()) */
        getStatuses: function() { return _stateNames; },

        /** Get logout feature code */
        getLogoutCode: function() { return _logoutCode; }
    };
